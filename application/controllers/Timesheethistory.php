...
class Timesheethistory extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('timesheethistory_m');
		$this->npwp=$this->session->userdata('npwp');
	}
	public function index(){
		$this->load->view('timesheet/show_timesheethistory_v');
	}
	...
	{	
		$bulantext= array('1' => 'Januari',
											...
											'12' => 'Desember');
		$npwp=$this->npwp;
		$table=$this->timesheethistory_m->getall($npwp);
		foreach ($table as $key => $value) {

			$tbody=array();
			$tbody[]=$key+1;
			$tbody[]=$bulantext["$value[bulan]"];
			$tbody[]=$value['tahun'];


			$tbody[]="<input type='hidden' name='npwp' value='$npwp'>
	...
	{	
		...
		if ($idtimesheet) {
			...
			$tfoot.="<th colspan='1'>Rp. <span class='number-format2'>$timesheettotal[totalope]</span></th>";
			$tfoot.="<th></th>";
			$tfoot.="</tr>";
			$data['tfoot'][]=$tfoot;
	...
	public function getDetailtimesheet($npwp,$bulan,$tahun){
		...
		query("select ts.npwp, concat('$bulantext[$bulan] $tahun')waktu, nama_user
	...
	public function printpage($print){
		if ($print=="timesheet1") {
			...
			if ($idtimesheet) {
				...
				$tfoot.="<th>$timesheettotal[totaljaker]</th>";
				$tfoot.="<th colspan='1'>Rp. <span class='number-format3'>$timesheettotal[totalope]</span></th>";
				$tfoot.="<th></th>";
				$tfoot.="</tr>";
				$data['tfoot'].=$tfoot;
